...
export default function parse(element, { document }) {
  // Support two source markups: trendsetters details/summary and WKND AEM cmp-accordion.
  const cmpItems = Array.from(element.querySelectorAll(':scope > .cmp-accordion__item, :scope .cmp-accordion__item'));
  if (cmpItems.length) {
    const cells = [];
    cmpItems.forEach((item) => {
      const titleEl = item.querySelector('.cmp-accordion__title');
      const title = titleEl ? titleEl.textContent.trim() : (item.querySelector('.cmp-accordion__button')
        ? item.querySelector('.cmp-accordion__button').textContent.trim() : '');
